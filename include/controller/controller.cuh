...
__device__ void main_loop(const globals &g, ::megakernel::state<config> &kvms) {
    ...
    for (kvms.instruction_index = 0, kvms.instruction_ring = 0;
        ...
        if (kvms.instruction_index == 0) {
            ...
            int last_opcode =
                kvms.all_instructions[last_instruction_ring].instructions[0] & 0xFF; // mask to low 8 bits for generic ISA
        ...
        int opcode = kvms.instruction()[0] & 0xFF; // mask to low 8 bits for generic ISA
        if (opcode == 0) {
