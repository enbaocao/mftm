...
__device__ inline unsigned int get_worker_id() {
    // For instruction fetch, we want an index into the first dimension of
    // the instructions buffer. Using blockIdx.x makes this independent of
    // which SM the block is scheduled on. The SM ID can be larger than the
    // number of instruction batches (often 1 in smoke/demo builds), which
    // leads to out-of-bounds reads and no-op execution.
    return blockIdx.x;
}
